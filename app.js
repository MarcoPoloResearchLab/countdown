  ...
  methods: {
    ...
    generateCalendarDays(yearValue, monthValue) {
      ...
      const parseDateStringToLocal = (dateStr) => {
        ...
        // Split the string and create date with numbers to ensure local midnight
        const [year, month, day] = dateStr.split("-").map(Number);
        // Note: month is 0-indexed for the Date constructor
        return new Date(year, month - 1, day);
      };

      const startDate = parseDateStringToLocal(this.startDate);
      const endDate = parseDateStringToLocal(this.endDate);

      // Leading days from previous month
      for (let dayIndex = 0; dayIndex < firstDay.getDay(); dayIndex++) {
        const tempDate = new Date(yearValue, monthValue, -dayIndex); // This is already local
        days.unshift({
          date: tempDate,
          otherMonth: true,
          isToday: false, // Not relevant for comparison fix
          // Compare using toDateString() - now it should work consistently
          isStartDate:
            startDate && tempDate.toDateString() === startDate.toDateString(),
          isEndDate:
            endDate && tempDate.toDateString() === endDate.toDateString(),
        });
      }

      // Days of the current month
      for (let dateNumber = 1; dateNumber <= lastDay.getDate(); dateNumber++) {
        const tempDate = new Date(yearValue, monthValue, dateNumber); // This is already local
        days.push({
          date: tempDate,
          otherMonth: false,
      ...
      if (leftoverCells < 7) {
        for (let dateNumber = 1; dateNumber <= leftoverCells; dateNumber++) {
          const tempDate = new Date(yearValue, monthValue + 1, dateNumber); // This is already local
          days.push({
            date: tempDate,
            otherMonth: true,
            isToday: false, // Not relevant for comparison fix
